...
```{r}
pl_proj_with_diff = tibble(celltype = celltype_assign, condition = cond_assign,emb = fit$embedding[1,]) %>%
...
```{r}
plot_assemble(
  add_plot(pl_proj_with_diff, x = 2, y = 0, width = 60, height = 30),
  width = 65, height = 35, units = "mm", show_grid_lines = FALSE,
  latex_support = TRUE, filename = "out/simulated_data_projected_with_diff.pdf"
)
pdftools::pdf_convert("out/simulated_data_projected_with_diff.pdf", dpi = 600,
                      filenames = "out/simulated_data_projected_with_diff.png")
```
