...
Claude summaries the above genes like this:
...
## Find our way around them, and arrange 
...
Find the combinations and the corresponding controls. `combs` is a dataframe with indices into the columns of `nmat`
...
combs
combsdf = data.frame(double = colnames(dx)[combs$double], single1 = colnames(dx)[combs$single1], single2 = colnames(dx)[combs$single2]) 
knitr::kable(combsdf)
```
...
fits = lapply(seq_len(nrow(combs)), function(i) {
  ...
  tibble(beta1 = coef(fit)[1], beta2 = coef(fit)[2], res = mean(residuals(fit)^2))
}) |> bind_rows() |> bind_cols(combs)
```
```{r}
#| label: showfit
#| fig.dim: !expr c(3.5, 3.5)
#| eval: true
ggplot(fits, aes(x = beta1, y = beta2, col = log(res))) + geom_point() + coord_fixed()
ggplot(filter(fits, res <= median(res)), aes(x = beta1, y = beta2)) + geom_point() + coord_fixed()
```