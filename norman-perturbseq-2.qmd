...
assert_that(!any(is_ctrl1)) |> invisible()
...
#| warning: false
pca2d = as_tibble(pca$x[, c("PC1", "PC2")], rownames = "condition") |> mutate(is_ctrl = is_ctrl2)
ggplot(pca2d, aes(x = PC1, y = PC2, label = condition, col = is_ctrl)) + scale_colour_manual(values = c(`TRUE` = "blue3", `FALSE` = "darkorange3")) + 
...
fits = lapply(seq_len(nrow(combs)), function(i) {
  ...
  print( with(df, sum(x1*x2)) |> signif(3) )
