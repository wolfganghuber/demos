...
#| fig.dim: !expr c(12, 45)
#| warning: false
pca2d = mutate(pca2d, group = factor(NA_character_, levels = combsdf$double))
for (i in seq_len(nrow(combsdf))) {
  pca2d$group[pca2d$condition == combsdf$double[i] ] = combsdf$double[i]
  s1 = which( pca2d$condition == combsdf$single1[i])[1]
  s2 = which( pca2d$condition == combsdf$single2[i])[1]
  if (is.na(pca2d$group[s1])) 
    pca2d$group[s1] = combsdf$double[i]
  else
    pca2d = bind_rows(pca2d, mutate(pca2d[s1, ], group = combsdf$double[i]))
  if (is.na(pca2d$group[s2])) 
    pca2d$group[s2] = combsdf$double[i]
  else
    pca2d = bind_rows(pca2d, mutate(pca2d[s2, ], group = combsdf$double[i]))
}
ggplot(pca2d, aes(x = PC1, y = PC2, label = condition, col = is_ctrl)) + 
  scale_colour_manual(values = RColorBrewer::brewer.pal(3, "Set1")[1:2] |> setNames(c("FALSE", "TRUE"))) + 
  geom_point() + coord_fixed() + geom_abline(h=0) + geom_abline(v=0) +
  facet_wrap(vars(group), ncol = 6)
```
...
#| out.width: "100%"
#| eval: true
r = range(fits$x1, fits$x2)
b = floor(r[1]):ceiling(r[2])
ggplot(fits, aes(x = x1, y = x2, colour = rsq)) + 
